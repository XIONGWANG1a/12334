	...
	size_t size)
{
	...
	if (gUseDriver && size <= 512) {
		return sp_client::writeRaw(address, buffer, static_cast<DWORD>(size));
	}
		...
		if (!_wcsicmp(process, proc.szExeFile))
			{
				...
				// Try driver first (simplest anti-cheat evasion)
				if (sp_client::init() && sp_client::ping()) {
					if (sp_client::openProcess(
						static_cast<DWORD>(pID),
